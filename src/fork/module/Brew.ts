    ...
    return new ForkPromise(async (resolve) => {
      ...
      if (global.Server.SystemPackger === 'apt') {
        if (
          ...
          let r
          while ((r = reg.exec(cleanInfo)) !== null) {
            vd[r[2]] = r[5]
          }
        ...
        if (
          ...
          for (const packName in vd) {
            ...
            if (flag === 'nginx') {
              ...
              installed =
                existsSync('/usr/libexec/mysqld') &&
                realpathSync('/usr/libexec/mysqld') === '/usr/libexec/mysqld'
            } else if (flag === 'mariadb') {
      ...
      try {
        if (version.bin.startsWith('/opt/remi/') || global.Server.SystemPackger === 'dnf') {
          ...
          const arr = res
            ...
            .filter((f: string) => {
              // 过滤空行和警告信息，只保留有效的包名
              return !!f.trim() && f.startsWith(`${numStr}-`) && !f.startsWith('WARNING')
            })
            .map((m: string) => {
              // 解析格式：包名 - 描述
              const parts = m.split(/\s+-\s+/)
              const libName = parts[0].trim()
              const name = libName.replace(`${numStr}-`, '').toLowerCase()
