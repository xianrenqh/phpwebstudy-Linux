  ...
  allInstalledVersions(flag: Array<string>, setup: any) {
    return new ForkPromise((resolve) => {
      ...
      const findFromMacPorts = async (type: string) => {
        ...
        const base = '/usr/'
        if (type === 'php') {
          const allBinFile = await getAllFileAsync(join(base, 'bin'), false)
          const allSbinFile = await getAllFileAsync(join(base, 'sbin'), false)

          const phpBins = allBinFile
            .filter((f) => f.startsWith('php') && (f.match(/php\d+\.\d+/) || []).length > 0)
            .map((f) => `bin/${f}`)

          const fpms = allSbinFile.filter((f) => f.startsWith('php-fpm')).map((f) => `sbin/${f}`)

          const versionMap: { [version: string]: SoftInstalled } = {}

          const findPhp = async (binFile: string) => {
            const bin = join(base, binFile)
            const match = binFile.match(/php(\d+\.\d+)/)
            if (!match) return
            const v = match[1]
            if (existsSync(bin)) {
              ...
              const item = {
                ...
                fpmBin: join(base, `sbin/php-fpm${v}`)
              }
              const key = v
              if (!versionMap[key]) {
                versionMap[key] = item as any
              }
            }
            return true
          }

          const findFpm = async (fpmFile: string) => {
            const bin = join(base, fpmFile)
            const match = fpmFile.match(/php-fpm(\d+\.\d+)/)
            if (!match) return
            const v = match[1]
            if (existsSync(bin)) {
              const { error, version } = await this.binVersion(bin, 'php-fpm')
              ...
              const key = v
              if (versionMap[key]) {
                versionMap[key].bin = bin
                versionMap[key].fpmBin = bin
                versionMap[key].version = version || v
                versionMap[key].num = num
                versionMap[key].enable = version !== null
              } else {
                const item = {
                  version: version || v,
                  bin,
          ...
          Object.values(versionMap).forEach((item) => {
            list.push(item)
          })
        } else if (type === 'nginx') {
          const allSbinFile = await getAllFileAsync(join(base, 'sbin'), false)
          ...
          const allBinFile = await getAllFileAsync(join(base, 'bin'), false)
          const mysqlBins = allBinFile.filter((f) => f === 'mysql')
          for (const binFile of mysqlBins) {
            const bin = join(base, 'bin', binFile)
            if (existsSync(bin)) {
              const { error, version } = await this.binVersion(bin, 'mysqld')
              const num = version ? Number(version.split('.').slice(0, 2).join('')) : null
              list.push({
                version,
                bin,
          ...
          const allBinFile = await getAllFileAsync(join(base, 'bin'), false)
          const redisBins = allBinFile.filter((f) => f.startsWith('redis-server'))
          for (const binFile of redisBins) {
            const bin = join(base, 'bin', binFile)
            if (existsSync(bin)) {
          ...
          const allBinFile = await getAllFileAsync(join(base, 'bin'), false)
          const pgBins = allBinFile.filter((f) => f.startsWith('psql'))
          for (const binFile of pgBins) {
            const bin = join(base, 'bin', binFile)
            if (existsSync(bin)) {
